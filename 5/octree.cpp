	...
	const Mesh & m, int level)
{
	...
	Box cBox[8];
	//ewww....
	cBox[0] = Box(mn,mid);//后边の左下角
	cBox[1] = Box(mn[0], mn[1],  mid[2], mid[0], mid[1], mx[2] );//前面の左下角
	cBox[2] = Box(mn[0], mid[1], mn[2],  mid[0], mx[1],  mid[2] );//后面の左上角
	cBox[3] = Box(mn[0], mid[1], mid[2], mid[0], mx[1],  mx[2] );//前面の左上角
	cBox[4] = Box(mid[0], mn[1],  mn[2],  mx[0],  mid[1], mid[2] );//后面の右下角
	cBox[5] = Box(mid[0], mn[1],  mid[2], mx[0],  mid[1], mx[2] );//前面の右下角
	cBox[6] = Box(mid[0], mid[1], mn[2],  mx[0],  mx[1],  mid[2] );//后面の右上角
	cBox[7] = Box(mid[0], mid[1], mid[2], mx[0],  mx[1],  mx[2] );//前面の右上角
